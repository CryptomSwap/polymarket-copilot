...
export async function GET() {
  try {
    ...
    const dashboard = {
      ...
      /** Execution failure containment: frozen assets, ambiguity counts. */
      ...
      diagnostics: {
        ...
        decisionTypesByAction: diagnostics.decisionTypesByAction ?? {},
        noopReasonsByCode: diagnostics.noopReasonsByCode ?? {},
        ...
        intentsBlockedByFreshness: diagnostics.intentsBlockedByFreshness ?? 0,
        freshnessBlockReasonCounts: diagnostics.freshnessBlockReasonCounts ?? {},
        ...
        runtimeReconciliationRuns: diagnostics.runtimeReconciliationRuns ?? 0,
        runtimeReconciliationFailures: diagnostics.runtimeReconciliationFailures ?? 0,
        lastRuntimeReconciliationAt: diagnostics.lastRuntimeReconciliationAt ?? null,
        lastRuntimeReconciliationStatus: diagnostics.lastRuntimeReconciliationStatus ?? null,
        driftDetectionsCount: diagnostics.driftDetectionsCount ?? 0,
        repairsAttemptedCount: diagnostics.repairsAttemptedCount ?? 0,
        repairsAppliedCount: diagnostics.repairsAppliedCount ?? 0,
        schedulerQueueHighWaterMark: diagnostics.schedulerQueueHighWaterMark ?? 0,
        schedulerDroppedEvents: diagnostics.schedulerDroppedEvents ?? 0,
        schedulerCoalescedEvents: diagnostics.schedulerCoalescedEvents ?? 0,
        schedulerLastEvaluationLatencyMs: diagnostics.schedulerLastEvaluationLatencyMs ?? null,
        schedulerOverloadPeriodCount: diagnostics.schedulerOverloadPeriodCount ?? 0,
        submitAmbiguousCount: diagnostics.submitAmbiguousCount ?? 0,
        cancelAmbiguousCount: diagnostics.cancelAmbiguousCount ?? 0,
        replaceAmbiguousCount: diagnostics.replaceAmbiguousCount ?? 0,
        executionVerificationRequiredCount: diagnostics.executionVerificationRequiredCount ?? 0,
      },
